...
if( !function_exists( 'sms_notifications_shortcode_variable' ) )
	{
		function sms_notifications_shortcode_variable( $body, $order )	{
			if( !$body || !is_object($order))
				return;
			...
			if( preg_match("/{SHOP_NAME}/i", $body) )
			{
				$SHOP_NAME = get_option( "blogname" );
				$body = @str_replace( "{SHOP_NAME}", $SHOP_NAME, $body );
			}
			
			if( preg_match("/{ORDER_NUMBER}/i", $body) )
			{
				$ORDER_NUMBER = isset( $order_id ) ? $order_id : "";
				$body = @str_replace( "{ORDER_NUMBER}", $ORDER_NUMBER, $body );
			}
			
			if( preg_match("/{ORDER_DATE}/i", $body) )
			{
			...
			if( preg_match("/{ORDER_STATUS}/i", $body) )
			{
				$ORDER_STATUS = @ucfirst($order->status);
				$body = @str_replace( "{ORDER_STATUS}", $ORDER_STATUS, $body );
			}
			
			if( preg_match("/{ORDER_ITEMS}/i", $body) )
			{
			...
			if( preg_match("/{BILLING_FNAME}/i", $body) )
			{
				$BILLING_FNAME = $order_custom_fields["_billing_first_name"][0];
				$body = @str_replace( "{BILLING_FNAME}", $BILLING_FNAME, $body );
			}
			
			if( preg_match("/{BILLING_LNAME}/i", $body) )
			{
				$BILLING_LNAME = $order_custom_fields["_billing_last_name"][0];
				$body = @str_replace( "{BILLING_LNAME}", $BILLING_LNAME, $body );
			}
			
			if( preg_match("/{BILLING_EMAIL}/i", $body) )
			{
				$BILLING_EMAIL = $order_custom_fields["_billing_email"][0];
				$body = @str_replace( "{BILLING_EMAIL}", $BILLING_EMAIL, $body );
			}
			
			if( preg_match("/{ORDER_AMOUNT}/i", $body) )
			{
				$ORDER_AMOUNT = $order_custom_fields["_order_total"][0];
				$body = @str_replace( "{ORDER_AMOUNT}", $ORDER_AMOUNT, $body );
			}
			
			if( preg_match("/{CURRENT_DATE}/i", $body) )
			{
			...
			if( preg_match("/{CURRENT_TIME}/i", $body) )
			{
				$wp_time_format = get_option( "time_format" );
				$CURRENT_TIME = date_i18n($wp_time_format, $current_date_time );
				$body = @str_replace( "{CURRENT_TIME}", $CURRENT_TIME, $body );
			}
...
class textlocal {
	...
	public function _sendRequest($command, $params = array())
	{
		...
		$message = __( 'Invalid API key! An error has occurred.', 'sms_notification' );
	...
	public function sendSms($body, $recipients = array())
	{
		...
		$settings['sms_sender_name'] = $options['senderid'];
        
		$admin_notification = $options['sms_admin_notification'];

		if($admin_notification == 'on' && !empty($options['adminnumber'])){
			$uri = $apiurl."send?&apiKey=".urlencode($settings['sms_auth_token'])."&sender=".urlencode($settings['sms_sender_name'])."&numbers=".urlencode(implode(',',$recipients))."&message=".urlencode($body);
		}else{
			$uri = $apiurl."send?&apiKey=".urlencode($settings['sms_auth_token'])."&sender=".urlencode($settings['sms_sender_name'])."&numbers=".urlencode($recipients)."&message=".urlencode($body);
		}
