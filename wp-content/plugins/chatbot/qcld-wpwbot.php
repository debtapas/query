 ...
 * Description: ChatBot is a WordPress ChatBot plugin to provide quick support and email functionality.
...
define('QCLD_wpCHATBOT_VERSION', '3.7.1');
...
class qcld_wb_Chatbot
{
    ...
    public function qcld_wb_chatbot_admin_menu()
    {
       /* add_submenu_page('wpcommerce',
		...
		add_submenu_page( 'wpbot-panel', esc_html('Help'), esc_html('Help'), 'manage_options','wpbot_help_page', 'wpbot_help_page_callback_func' );
    }
...
function qc_wpbot_simple_response_intent(){
	global $wpdb;
	$table = $wpdb->prefix.'wpbot_response';
	$results = $wpdb->get_results("SELECT `intent` FROM `wp_wpbot_response` WHERE 1 and `intent` !=''");
	$response = array();
	if(!empty($results)){
		foreach($results as $result){
